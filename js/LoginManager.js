...
class LoginManager {
  ...
  _init() {
    ...
    for (let name of this._plugins) {
      ...
      $plugin.on("ready", () => {
        ready_count++;
        if (ready_count == this._plugins.length) $(this).trigger("ready");
      });
  ...
  auth(username = "", password = "", callback) {
    if (!this.lightdm) {
      log.warn(
        "Cannot attempt login because lightdm is missing the " +
    ...
    let auth_cb = () => {
      // called as a result of authenticate
      this.lightdm.respond(password);
    };
    let auth_complete_cb = () => {
      // called as a result of respond
      try {
        // attempt to cache selcted username
  ...
  fillUserSelect($el) {
    ...
    for (let s of this.lightdm.users)
      $el.append(
        "<option value=" + s.username + ">" + s.display_name + "</option>"
      );
  ...
  fillSessionSelect($el) {
    ...
    try {
      ...
      if (session) $el.val(session.name);
    } catch (e) {}
   ...
   */
  shutdown() {
    if (this.lightdm.can_shutdown) this.lightdm.shutdown();
    else {
  ...
  restart() {
    if (this.lightdm.can_restart) this.lightdm.restart();
    else {
  ...
  hibernate() {
    if (this.lightdm.can_hibernate) this.lightdm.hibernate();
    else {
  ...
  suspend() {
    if (this.lightdm.can_suspend) this.lightdm.suspend();
    else {
