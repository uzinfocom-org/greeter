...
if (
  ...
  window.navigator.language === "en-CA" ||
  ...
  window.navigator.language === "fr-CA"
) {
  ...
  text[0] = "Tizimga kirish uchun Enter ni bosing";
  text[1] = "Xush Kelibsiz";
  text[2] = "Sessiyani Tanlang";
  text[3] = "Foydalanuvchi";
  text[4] = "Maxfiy Kalit";
}
...
class SplashScreen {
    ...
    this._DEF_OPT = {
      ...
      content: {
        ...
        html: [
          {
            html:
              "<text style='display: none' class='active-appear'>" +
              text[0] +
              "</text>",
            css: {
              "margin-top": "40vh",
  ...
  _init() {
    ...
    if (typeof options == "object") {
      ...
      if (options.filter == true) this.$img.addClass("filter");
    ...
    this.clock = setInterval(() => {
      ...
      if (!this._isActive()) $(this).trigger("inactive");
    }, 500);
    ...
    this.$el
      ...
      .mousemove((e) => {
        if (!this._isActive()) $(this).trigger("active", e);
      });
  ...
  _initContent(content) {
    for (let content_type in content) {
      if (content_type == "clock") this._initClock(content[content_type]);
      else if (content_type == "html") this._initHTML(content[content_type]);
      else log.warn("Specified content " + content_type + " is not valid.");
    }
  }
  ...
  _startClock($clock, opts) {
    ...
    if (!Array.isArray(opts.css)) opts.css = [opts.css];
    ...
    if (typeof opts["parent-css"] == "object") $clock.css(opts["parent-css"]);
  ...
  _keyHandler(e) {
    ...
    if (this._state == "open") clearTimeout(this.resetTimeout);
